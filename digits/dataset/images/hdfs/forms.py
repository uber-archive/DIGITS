...
class HdfsImageDatasetForm(ImageDatasetForm):
    """
    ...
    prebuilt_train_images = wtforms.StringField('Training H5 File HDFS Directory ',
            validators=[
                validate_required_iff(method='prebuilt'),
                validate_lmdb_path,
                ]
            )
    prebuilt_train_labels = wtforms.StringField('H5 file fields',
            default='data,label',
            )
    prebuilt_val_images = wtforms.StringField('Validation H5 File HDFS Directory',
            validators=[
                validate_lmdb_path,
                ]
            )
    prebuilt_val_labels = wtforms.StringField('NA',
            default='This field is inactive'
            )
