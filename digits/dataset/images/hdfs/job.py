...
@subclass
class HdfsImageDatasetJob(ImageDatasetJob):
    """
    ...
    def __init__(self, **kwargs):
        ...
        super(HdfsImageDatasetJob, self).__init__(**kwargs)
        self.pickver_job_dataset_image_generic = PICKLE_VERSION

    def __setstate__(self, state):
        super(HdfsImageDatasetJob, self).__setstate__(state)
        self.pickver_job_dataset_image_generic = PICKLE_VERSION
    ...
    def analyze_db_task(self, stage):
        ...
        if stage == constants.TRAIN_DB:
            ...
            return None
        for t in self.tasks:
            if isinstance(t, tasks.HdfsDbTask) and s in t.name().lower():
                return t
        return None
    ...
    def analyze_db_tasks(self):
        ...
        """
        return [t for t in self.tasks if isinstance(t, tasks.HdfsDbTask)]
